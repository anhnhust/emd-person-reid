...
class Market1501(object):
    ...
    """
    dataset_dir = '/home/micalab/AlignedReID/data/market1501'

    def __init__(self, root='data', **kwargs):
        self.dataset_dir = osp.join(root, self.dataset_dir)
        self.train_dir = osp.join(self.dataset_dir, 'bounding_box_train')
        self.query_dir = osp.join(self.dataset_dir, 'query')
        self.gallery_dir = osp.join(self.dataset_dir, 'gbounding_box_test')
...
class Market1501_Partial(object):
    ...
    # images: 12936 (train) + 3368 (query) + 15913 (gallery)
    """
    dataset_dir = "/home/micalab/AlignedReID/data/market1501_partial"
...
class Viper(object):
    ...
    dataset_dir = "/home/anhhoang/AlignedReID/AlignedReID/data/Viper/"
    ...
    def __init__(self, root='',split_id=0, **kwargs):
        ...
        self.split_path = osp.join(self.dataset_dir, 'split.json')
        ...
        num_train_pids=int(len(train)/2)
        num_query_pids=len(query)
        num_gallery_pids=len(gallery)
        num_train_imgs= len(train)
        num_query_imgs= num_query_pids
        num_gallery_imgs=num_gallery_pids
        num_total_pids =num_train_pids + num_query_pids
        num_total_imgs = num_train_imgs + num_query_imgs + num_gallery_imgs
        print("=> Viper loaded")
        print("Dataset statistics:")
        ...
        self.num_train_pids = len(train)
        self.num_query_pids = len(query)
        self.num_gallery_pids = len(gallery)
        #super(Viper, self).__init__(train, query, gallery, **kwargs)
...
class Partial_REID(object):
    ...
    def __init__(self, root='data', **kwargs):
        self.dataset_dir = osp.join(root, self.dataset_dir)
        self.train_dir = osp.join(self.dataset_dir, 'partial_body_images')
        self.test_dir = osp.join(self.dataset_dir, 'whole_body_images')
        self.list_train_path = osp.join(self.dataset_dir, 'Probe.txt')
        self.list_val_path = osp.join(self.dataset_dir, 'Gallery.txt')
        #self.list_query_path = osp.join(self.dataset_dir, 'MSMT17_V1/list_query.txt')
        #self.list_gallery_path = osp.join(self.dataset_dir, 'MSMT17_V1/list_gallery.txt')
...
__img_factory = {
    ...
    'occludereid': OccludedREID,
    'viper' : Viper,
    'viper2':Viper2,
    'DukeMTMC_partial':DukeMTMC_partial,
    'cuhk03_partial':cuhk03_partial
}
